...
def file_finder():
    ...
    where = input()
    file_address = 'TYPE THE FOLDER ADDRESS - THE NAME OF THE FOLDER ITSELF' + where
    ...
    for essay in txt_files:
        ...
        with open(essay_address, 'r', encoding='utf-8') as file:
            ...
            for sentence in sentences:
                sentence = re.sub('<[,;:%()-]\sPUNCT>', '', sentence) # because we don't accaunt for it in this model
                ...
                perfect_contexts = re.findall(perfect_context, sentence)
                  if len(perfect_contexts) != 0:
                    ...
                    trigger_3 = re.compile("<in\sADP><[a-zA-z]+\s[A-Z]+><[a-zA-z]+\s[A-Z]+><[a-zA-z]+\s[A-Z]+><[a-zA-z\d]+\sNUM>")
                    trigger_4 = re.compile("<in\sADP><[a-zA-z]+\s[A-Z]+><[a-zA-z]+\s[A-Z]+><[a-zA-z]+\s[A-Z]+><[a-zA-z]+\s[A-Z]+><[a-zA-z\d]+\sNUM>")
                    trigger_5 = re.compile("<in\sADP><[a-zA-z]+\s[A-Z]+><[a-zA-z]+\s[A-Z]+><[a-zA-z]+\s[A-Z]+><[a-zA-z]+\s[A-Z]+><[a-zA-z]+\s[A-Z]+><[a-zA-z\d]+\sNUM>")
